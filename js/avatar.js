 ...
 * Maakt een unieke, herkenbare SVG-illustratie per speler op basis van zijn id.
 * Geen externe requests, geen auteursrecht. Zelfde id => zelfde avatar.
 *
 * window.makeAvatar(player, { showNumber, size }) => SVG-string
 */
(function () {
  function hash(str) {
    let h = 2166136261 >>> 0;
  ...
  function makeAvatar(player, opts) {
    opts = opts || {};
    const size = opts.size || 120;
    const r = makeRng(hash(player.id || player.name || "x"));
    ...
    // Optioneel: echte foto i.p.v. avatar (drop-in voor later).
    if (player.image) {
      return `<svg viewBox="0 0 100 100" width="${size}" height="${size}" role="img" aria-label="Foto">
        <defs><clipPath id="c-${player.id}"><circle cx="50" cy="50" r="50"/></clipPath></defs>
        <image href="${player.image}" x="0" y="0" width="100" height="100"
               preserveAspectRatio="xMidYMid slice" clip-path="url(#c-${player.id})"/>
      </svg>`;
    }

    const numberBadge = opts.showNumber
      ? `<g>
           ...
           <text x="82" y="88" text-anchor="middle" font-size="16" font-weight="700"
                 fill="#fff" font-family="system-ui, sans-serif">${player.number}</text>
         </g>`
      : "";
    ...
    return `<svg viewBox="0 0 100 100" width="${size}" height="${size}" role="img" aria-label="Spelersavatar">
      ...
      ${hasGlasses ? `<g stroke="#333" stroke-width="1.6" fill="none">
